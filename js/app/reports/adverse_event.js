		(function () {
			define(["jquery", "d3", "jnj/chart", "common", "datatables", "datatables-colvis"], function ($, d3, jnj_chart, common) {
				...
				$(document).on('click', '#adverse_event_table tbody tr', function () {
					$('#adverse_event_table tbody tr.selected').removeClass('selected');
					$(this).addClass('selected');
					id = datatable.data()[datatable.row(this)[0]].concept_id;
					concept_name = datatable.data()[datatable.row(this)[0]].adverse_event_name;
					adverse_event.drilldown(id, concept_name);
				});
...
//				adverse_event.drilldown = function (concept_id, concept_name) {
...
//					$.ajax({
//						type: "GET",
//						url: getUrlFromDataCollection(page_vm.datasource(), "adverse_events", concept_id),
...
//							});
//
//							// adverse_events by type distribution visualization
//							var boxplot = new jnj_chart.boxplot();
//							bpseries = [];
...
//							}
//
//							boxplot.render(bpseries, "#reportVisitOccurrences #adverse_eventDurationByType", 500, 300, {
//								yLabel: 'Duration'
//							});
//
//							// prevalence by month
...
//							prevalenceByMonth.render(byMonthSeries, "#reportVisitOccurrences #adverse_eventPrevalenceByMonth", 1000, 300, {
...
//				}
				...
				adverse_event.render = function (datasource) {
					format_pct = d3.format('.2%');
					format_fixed = d3.format('.2f');
					format_comma = d3.format(',');

					$('#reportStudyDataDensity #AdverseEvent svg').remove();
					...
					$.ajax({
						type: "GET",
						url: getUrlFromData(datasource, 'adverse_event_treemap'),
						...
						success: function (data) {
							...
							var table_data = normalizedData.CONCEPT_PATH.map(function (d, i) {
								...
								return {
									...
									adverse_event_name: conceptDetails[0],
							...
							datatable = $('#adverse_event_table').DataTable({
								order: [2, 'desc'],
								dom: 'Clfrtip',
								data: table_data,
								columns: [
...
//							$('#reportStudyAdverseEvent').show();
							$('#reportStudyDataDensity #AdverseEvent').show();

							tree = buildHierarchyFromJSON(data, threshold);
							var treemap = new jnj_chart.treemap();
//							treemap.render(tree, '#reportStudyAdverseEvent #treemap_container', width, height, {
							treemap.render(tree, '#reportStudyDataDensity #AdverseEvent #treemap_container', width, height, {
								onclick: function (node) {
									adverse_event.drilldown(node.id, node.name)
								},
