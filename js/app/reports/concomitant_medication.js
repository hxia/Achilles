		(function () {
			define(["jquery", "d3", "jnj/chart", "common", "datatables", "datatables-colvis"], function ($, d3, jnj_chart, common) {
				var concomitant_medication = {};
				var threshold;
				var datatable;

				// bind to all matching elements upon creation
				$(document).on('click', '#concomitant_medication_table tbody tr', function () {
					$('#concomitant_medication_table tbody tr.selected').removeClass('selected');
					$(this).addClass('selected');
					id = datatable.data()[datatable.row(this)[0]].concept_id;
					concept_name = datatable.data()[datatable.row(this)[0]].concomitant_medication_name;
					concomitant_medication.drilldown(id, concept_name);
				});
				...
				concomitant_medication.drilldown = function (concept_id, concept_name) {
					$('.drilldown svg').remove();
					$('#adverseEventDrilldownTitle').text(concept_name);
					$('#reportAdverseEventDrilldown').removeClass('hidden');

					$.ajax({
						type: "GET",
						url: getUrlFromDataCollection(page_vm.datasource(), "concomitant_medications", concept_id),
						success: function (data) {
							...
							// concomitant_medications by type distribution visualization
							var boxplot = new jnj_chart.boxplot();
							bpseries = [];
							...
							boxplot.render(bpseries, "#reportVisitOccurrences #concomitant_medicationDurationByType", 500, 300, {
								yLabel: 'Duration'
							});
							...
							prevalenceByMonth.render(byMonthSeries, "#reportVisitOccurrences #concomitant_medicationPrevalenceByMonth", 1000, 300, {
				...
				concomitant_medication.render = function (datasource) {
					format_pct = d3.format('.2%');
					format_fixed = d3.format('.2f');
					format_comma = d3.format(',');

//					$('#reportConcomitantMedication svg').remove();
					$('#reportStudyDataDensity #ConcomitantMedication svg').remove();
					...
					$.ajax({
						type: "GET",
						url: getUrlFromData(datasource, 'concomitant_medication_treemap'),
						...
						success: function (data) {
							...
							var table_data = normalizedData.CONCEPT_PATH.map(function (d, i) {
								...
								return {
                                    ...
                                    concomitant_medication_name: conceptDetails[0],
							...
							datatable = $('#concomitant_medication_table').DataTable({
								order: [2, 'desc'],
								dom: 'Clfrtip',
								data: table_data,
								columns: [
									...
									{
										data: 'concomitant_medication_name'
									},
									{
										data: 'num_persons',
										className: 'numeric'
...
//							$('#reportStudyConcomitantMedication').show();
//							$('#reportStudyDataDensity #ConcomitantMedication').show();
							$('#reportStudyDataDensity').show();

							tree = buildHierarchyFromJSON(data, threshold);
							var treemap = new jnj_chart.treemap();
//							treemap.render(tree, '#reportStudyConcomitantMedication #treemap_container', width, height, {
							treemap.render(tree, '#reportStudyDataDensity #ConcomitantMedication #treemap_container', width, height, {
								onclick: function (node) {
									concomitant_medication.drilldown(node.id, node.name)
								},
